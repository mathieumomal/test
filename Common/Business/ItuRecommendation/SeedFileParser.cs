            ...
            try
            {
                using (Stream fileStream = new FileStream(filePath, FileMode.Open))
                {
                    using (var memoryStream = new MemoryStream())
                    {
                        ...
                        if (wordProcessingDocument.MainDocumentPart.StyleDefinitionsPart.RootElement.Elements<Style>().Any(y => y.StyleId.Value.ToLower().StartsWith("titre") || y.StyleId.Value.ToLower().StartsWith("heading")))
                        {
                            ...
                            foreach (var paragraph in paragraphs)
                            {
                                ...
                                if (paragraph.Descendants().OfType<ParagraphStyleId>().Any(x => x.Val.ToString().ToLower().StartsWith("titre") || x.Val.ToString().ToLower().StartsWith("heading")))
                                {
                                    ...
                                    {
                                        var header = paragraph.InnerText.ToLower().Split(new string[] { "tr", "ts" }, StringSplitOptions.None);
                                        ...
                                        if (header.Count() == 2)
                                        {
                                            ...
                                            if (match.Success)
                                            {
                                                ...
                                                if (paragraph.Descendants().Any(x => x.LocalName == "tab"))
                                                {
                                                    ...
                                                    if (clauseText.ToLower() != paragraphTextWithTabCharacter.ToLower().Split('\t')[0].Trim())
                                                        isValidClause = false;
                                                }
